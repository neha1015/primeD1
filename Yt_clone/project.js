...
const handleHover = (e, idx) => {
    const thumbnails = dummyData[idx].videoThumbnails;
    if (thumbnails.length > 1) {
        e.target.src = thumbnails[1].url; 
    }
};
...
const showSearch = (data) => {
    ...
    if (!data.contents || data.contents.length === 0) {
        mainContainer.innerHTML = "<p>No videos found.</p>";
        return;
    }
    ...
    data.contents.forEach((item) => {
        if (item.type === "video") {
            ...
            videoElement.innerHTML = `
                <img src="${video.thumbnails[0].url}" alt="${video.title}" width="300">
                <h3>${video.title}</h3>
